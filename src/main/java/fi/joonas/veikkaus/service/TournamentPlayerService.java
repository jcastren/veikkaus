...
public class TournamentPlayerService {
	...
	public Long insert(String tournamentTeamId, String playerId, String goals) throws VeikkausDaoException {
		...
		if (player == null) {
			throw new VeikkausDaoException("Player with id: " + playerId + " wasn't found, insert failed");
		}

		return tournamentPlayerDao.save(new TournamentPlayer(tournamentTeam, player, Integer.parseInt(goals))).getId();
	}
	
	public Long modify(String id, String tournamentTeamId, String playerId, String goals) throws VeikkausDaoException {
		TournamentPlayer tournamentPlayer = tournamentPlayerDao.findOne(Long.valueOf(id));
		if (tournamentPlayer == null) {
			throw new VeikkausDaoException("tournamentPlayer with id: " + id + " wasn't found, modify failed");
		}
		...
		Player player = playerDao.findOne(Long.valueOf(playerId));
		if (player == null) {
			throw new VeikkausDaoException("Player with id: " + playerId + " wasn't found, modify failed");
		}
